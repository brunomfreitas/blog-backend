...
describe('[HTTP] Post routes', () => {
	...
	it('POST /post deve criar um post (201)', async () => {
		...
		const createdByResp = await request(app).post('/person').set('Authorization', `Bearer ${token}`).send(createdBy);
	...
	// 		status: true
	// 	};
	// 	const createdByResp = await request(app).post('/person').set('Authorization', `Bearer ${token}`).send(person);
	// 	const createdById = createdByResp.body.id;
	...
	// 	console.log('Mensagem 1 criada: ', createResp.body);

	// 	// Agora vamos atualizar
	// 	const id = createResp.body.id;

	// 	// cria um status
	// 	const status2 = { name: 'Finalizado' };	
	...
	// 	console.log('Mensagem 1 pre-atualizacao: ', update);

	// 	const updateResp = await request(app).put(`/post/${id}`).set('Authorization', `Bearer ${token}`).send(update);		

	// 	console.log('Mensagem 1 atualizada: ', updateResp.body);
